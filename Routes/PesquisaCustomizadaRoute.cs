...
public static class PesquisaCustomizadaRoute
{
    public static void PesquisaCustomizadaRoutes(this WebApplication app)
    {
        ...
        // 1️⃣ POST - adicionar preferência
        ...
        // 2️⃣ GET - listar todas as preferências de um usuário
        ...
        // 3️⃣ PUT - atualizar uma preferência existente
        ...
        // 4️⃣ DELETE - remover preferência
        ...
        // 5️⃣ GET - listar artigos com base nas preferências do usuário na página
        route.MapGet("/artigos/{usuarioId:int}todos_artigos", async (int usuarioId, AppDbContext context) =>
{
    var preferencias = await context.PesquisasCustomizadas
                                    .Where(p => p.UsuarioId == usuarioId)
                                    .ToListAsync();
    ...
    // Função local para extrair IDs de forma segura
    ...
    // Buscar artigos que correspondam a qualquer preferência
    ...
    var result = artigos.Select(a => new
    {
        a.Id,
        a.Titulo,
        a.Imagem,
